...
def create_character_from_state(world_state: dict, name: str, game_map: GameMap) -> AutonomousCharacter:
    ...
    # Initialize character position in world state
    game_map.world_state.set_character_position(name, character_house)
    
    # Initialize character in world state characters dict
    game_map.world_state.characters[name] = {
        'name': name,
        'position': {'x': character_house.x, 'y': character_house.y},
        'online': True,
...
def main():
    ...
    logger = logging.getLogger("AliceClient")
    ...
    try:
        ...
        character = create_character_from_state(world_state, "Alice", game_map)
        ...
        logger.info("Alice successfully connected and registered")
        ...
        logger.error(f"Error in Alice client: {str(e)}", exc_info=True)
    finally:
